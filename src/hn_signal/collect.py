...
def _deduplicate(stories: list[Story]) -> list[Story]:
    # Group by normalized URL
    url_groups: dict[str, Story] = {}
    no_url: list[Story] = []
    ...
    for story in stories:
        ...
        if norm in url_groups:
            ...
            if len(story.body or "") > len(existing.body or ""):
    ...
    for orphan in no_url:
        ...
        for existing in merged:
            ...
            if ratio > 0.8:
                ...
                if len(orphan.body or "") > len(existing.body or ""):
    ...
    while i < len(merged):
        ...
        while j < len(merged):
            ...
            if ratio > 0.8:
                ...
                if len(merged[j].body or "") > len(merged[i].body or ""):
...
def _rank(stories: list[Story]) -> list[Story]:
    today = date.today()
